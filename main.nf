...
workflow {
    ...
    log.info "  Use existing results: ${params.use_existing_results}"
    ...
    //////////////////////////////////////////
    if (params.run_magma) {
        if (params.use_existing_results) {
            ...
            if (params.run_empirical) {
                ...
                magma_for_empirical = magma_real_existing.combine(
                    magma_random_existing,
                    by: [0, 2]  // Join by trait and method
                    ...
                    tuple(trait, "magma_${rand_method}", real_file, random_dir)
                }
            ...
            if (params.run_empirical) {
                ...
                ).map { trait, rand_method, real_result_file, random_dir ->
    ...
    if (params.run_prset) {
        if (params.use_existing_results) {
            log.info "Using existing PRSet results"
            def (prset_real_existing, prset_random_existing) = create_prset_entry_channels()
            
            if (params.run_empirical) {
                prset_for_empirical = prset_real_existing.combine(
                    prset_random_existing,
                    by: [0, 2]  // Join by trait and method
                ).map { trait, real_file, rand_method, random_files ->
                    def random_dir = "${params.outdir}/prset_random/${rand_method}/${params.background}/${trait}"
                    tuple(trait, "prset_${rand_method}", real_file, random_dir)
                }
