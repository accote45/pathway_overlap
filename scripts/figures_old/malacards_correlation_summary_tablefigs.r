...
# Config
# ------------------------------------------------------------------------------
INPUT_ROOT <- "/sc/arion/projects/psychgen/cotea02_prset/geneoverlap_nf/results/malacards_correlation"
TOOL_BASE  <- "magma"
...
parse_one <- function(f) {
  bn <- basename(f)
  mm <- regmatches(bn, regexec("^(.+?)_([A-Za-z0-9]+)_malacards_rank_correlation_summary\\.csv$", bn))[[1]]
...
# Ensure "bmi" appears as "BMI" (used for dumbbell figures only)
fix_bmi <- function(x) {
  gsub("\\bbmi\\b", "BMI", x, ignore.case = TRUE, perl = TRUE)
}
...
make_methods_dumbbellplot <- function(subset_name, alpha_filter, title_suffix, filter_by_any_method = FALSE) {
  ...
  df0 <- df %>%
    transmute(
      trait_label  = fix_bmi(label_trait(trait)),
      method_id    = method,
      rho          = rank_mean_score_correlation,
      pval         = rank_mean_score_pvalue
    )
  ...
  # Build dumbbell data for the two comparisons
  dumb_dat <- bind_rows(
    dfw %>%
      transmute(
        trait_label,
    ...
    mutate(
      sig_label = dplyr::case_when(
        pval_bw < 0.05/(n_traits*length(METHODS3)*length(SUBSETS)) ~ "***",
        pval_bw < 0.001                                            ~ "**",
        pval_bw < 0.05                                             ~ "*",
        TRUE                                                       ~ ""
      )
