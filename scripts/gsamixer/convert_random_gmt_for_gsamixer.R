...
# 2. Retrieve hg19 positions for Ensembl IDs from GTF
gtf <- import(gtf_path)
...
write_tsv(baseline, paste0(output_prefix, "_baseline.txt"))
...
write_tsv(full_gene_rows, paste0(output_prefix, "_full_gene.txt"))
...
write_tsv(full_gene_set_rows, paste0(output_prefix, "_full_gene_set.txt"))

cat("Converted random GMT to GSA-MiXeR format:", gmt_path, "\n")