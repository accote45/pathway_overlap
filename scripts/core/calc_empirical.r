...
# Verify beta column exists (enrichment for gsamixer); PascalX has no beta column
if (!is.null(beta_col) && !beta_col %in% colnames(real_data)) {
...
process_random_files <- function(file_batch, path_col, p_col, beta_col, valid_pathways) {
  ...
  for (file_idx in seq_along(file_batch)) {
    ...
    tryCatch({
      ...
      # PascalX random files are plain .csv (but NOT .go_test_enrich.csv)
      is_pascalx <- endsWith(file, ".csv") && !is_gsamixer
      ...
      # Read the file
      if (is_magma || is_prset || is_gsamixer || is_pascalx) {
        ...
        temp_data <- tryCatch({
          if (is_magma) {
            ...
            # Try fread for non-MAGMA files (PRSet, GSA-Mixer, PascalX)
        ...
        # Check if required columns exist (beta_col is NULL for PascalX and collapses out)
        if (is.null(p_col)) {
        ...
        if (cols_exist) {
          # Extract only the columns we need (NULL beta_col drops out automatically)
          if (is.null(p_col)) {
            # For GSA-Mixer, we don't need p-values
            subset_data <- as.data.frame(temp_data)[, c(path_col, beta_col), drop = FALSE]
          } else {
            subset_data <- as.data.frame(temp_data)[, c(path_col, p_col, beta_col), drop = FALSE]
          }
  ...
  if (length(result_list) > 0) {
    ...
    # Rename columns to standardized names (name-based, NULL-safe)
    ...
    if (!is.null(beta_col) && beta_col %in% col_names) {
...
if (length(random_files) > 100) {
  ...
  random_chunks <- mclapply(chunks, process_random_files,
                          ...
                          mc.cores = num_cores)
  random_data <- rbindlist(random_chunks[!sapply(random_chunks, is.null)], fill = TRUE)
} else {
  ...
  for (i in seq_along(random_files)) {
    tryCatch({
      ...
      # PascalX random files are plain .csv (but NOT .go_test_enrich.csv)
      is_pascalx_rand <- endsWith(current_file, ".csv") && !is_gsamixer_rand

      # Only process files that match our criteria
      if (is_magma_rand || is_prset_rand || is_gsamixer_rand || is_pascalx_rand) {
        if (is_gsamixer_rand) {
          temp_data <- fread(current_file, header = TRUE)
          # Filter GSA-Mixer data to only include valid pathways from GMT
          temp_data <- as.data.frame(temp_data[temp_data[[pathway_col]] %in% valid_pathways])
        } else if (is_pascalx_rand) {
          temp_data <- as.data.frame(fread(current_file, header = TRUE))
        } else {
        ...
        # beta_col is NULL for PascalX and collapses out of the column vectors)
        if (is.null(pval_col)) {
          # For GSA-Mixer, we only need pathway and beta columns
          if (all(c(pathway_col, beta_col) %in% colnames(temp_data))) {
            valid_file_count <- valid_file_count + 1
            temp_data[["random_iter"]] <- valid_file_count
            random_data_list[[valid_file_count]] <- temp_data[, c(pathway_col, beta_col, "random_iter"), drop = FALSE]
          }
        } else if (all(c(pathway_col, pval_col, beta_col) %in% colnames(temp_data))) {
          valid_file_count <- valid_file_count + 1
          temp_data[["random_iter"]] <- valid_file_count
          random_data_list[[valid_file_count]] <- temp_data[, c(pathway_col, pval_col, beta_col, "random_iter"), drop = FALSE]
        }
      }
    }, error = function(e) {
  ...
  # Combine random data (still using original tool-specific column names;
  # standardization happens centrally below)
  random_data <- rbindlist(lapply(random_data_list, as.data.table), fill = TRUE)
...
# -------------------------------------------------------------------------
# Centralized, name-based standardization of random data columns.
# Handles both the parallel path (already renamed) and the sequential path
# (still tool-specific names), for all tools including PascalX.
# -------------------------------------------------------------------------
if (!"pathway_name" %in% names(random_dt) && pathway_col %in% names(random_dt)) {
  setnames(random_dt, pathway_col, "pathway_name")
}
if (!"p_value" %in% names(random_dt) && !is.null(pval_col) && pval_col %in% names(random_dt)) {
  setnames(random_dt, pval_col, "p_value")
}
if (!"beta_value" %in% names(random_dt) && !is.null(beta_col) && beta_col %in% names(random_dt)) {
  setnames(random_dt, beta_col, "beta_value")
}
# Ensure dummy columns exist for downstream consistency
if (!"p_value" %in% names(random_dt)) random_dt[, p_value := NA_real_]
if (!"beta_value" %in% names(random_dt)) random_dt[, beta_value := NA_real_]

# Add standardized column names to real data (NULL-safe for PascalX)
real_dt[, `:=`(
  pathway_name_std = get(pathway_col),
  p_value_std = if (!is.null(pval_col)) get(pval_col) else NA_real_,
  beta_value_std = if (!is.null(beta_col)) get(beta_col) else NA_real_
)]
...
# Calculate beta statistics in one go (guarded for tools without beta values)
if ("beta_value" %in% names(random_dt) && any(!is.na(random_dt$beta_value))) {
  ...
  # PascalX (or any tool with no beta values): no effect sizes available
  beta_stats <- random_dt[, .(
    mean_beta_perm = NA_real_,
    sd_beta_perm = NA_real_,
    n_perms = .N
  ), by = pathway_name]
}
...
results_dt[, `:=`(
  std_effect_size = {
    # Calculate standardized effect size (NA when no beta values, e.g. PascalX)
