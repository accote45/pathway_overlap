...
MALACARDS_FOLDER <- "/sc/arion/projects/psychgen/cotea02_prset/geneoverlap_nf/results/opentargets_correlation"
...
if (nrow(dat) == 0) stop("No data found for OT analysis.")
...
if (nrow(dat) == 0) stop("No filtered data for OT analysis.")
...
plot_dat <- dat %>%
  ...
  plot_dat <- plot_dat %>%
  mutate(
    tool_base = ifelse(grepl("_prset", trait_label), "prset", "magma"),
    trait_label = tolower(gsub("_prset", "", trait_label))
  )
...
ggsave("dumbbell_ot_twopanel.pdf", plot = p, width = 12, height = 4.2)