...
def main():
    ...
    # and turn a completed run (output CSV written and closed above) into a non-zero
    # process exit. Bypass the teardown with a hard exit so the process reports the
    # real, successful status.
    sys.stdout.flush()
    sys.stderr.flush()
    os._exit(0)
